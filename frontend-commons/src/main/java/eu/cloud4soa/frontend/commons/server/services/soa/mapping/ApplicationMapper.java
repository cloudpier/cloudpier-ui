...
public class ApplicationMapper extends
	...
	protected ApplicationModel readFrom(ApplicationInstance soaInstance) {
		...
		// sla
		for (ServiceQualityInstance qosInstance : soaInstance.getServiceQualities())
			...
			else if (qosInstance instanceof CPULoadInstance)
				applicationModel.set(ApplicationModel.SLA_CPU_LOAD,
						((CPULoadInstance) qosInstance).getHasPercentage());
			else if (qosInstance instanceof CloudResponseTimeInstance)
				applicationModel.set(ApplicationModel.SLA_CLOUD_RESPONSE_TIME,
						((CloudResponseTimeInstance) qosInstance).getMaxValueMs());
			else if (qosInstance instanceof ContainerResponseTimeInstance)
				applicationModel.set(ApplicationModel.SLA_CONTAINER_RESPONSE_TIME,
						((ContainerResponseTimeInstance) qosInstance).getMaxValueMs());
			else if (qosInstance instanceof MemoryLoadInstance)
				applicationModel.set(ApplicationModel.SLA_MEMORY_LOAD,
						((MemoryLoadInstance) qosInstance).getMaxValueMB());
	...
	protected ApplicationInstance writeTo(ApplicationInstance soaInstance,
        ...
        //CPU Load
        Float cpuLoad = frontendModel.<Float> get(ApplicationModel.SLA_CPU_LOAD);
        CPULoadInstance cpuLoadInstance = null;
        for (ServiceQualityInstance qosInstance : soaInstance
                .getServiceQualities()) {
            if (qosInstance instanceof CPULoadInstance) {
            	cpuLoadInstance = ((CPULoadInstance) qosInstance);
            }
        }
        if (cpuLoadInstance == null) {
        	cpuLoadInstance = new CPULoadInstance();
        }
        cpuLoadInstance.setHasPercentage(cpuLoad);
        serviceQualityInstances.add(cpuLoadInstance);
      
        
        //Cloud Response Time
        Float cloudResponseTime = frontendModel.<Float> get(ApplicationModel.SLA_CLOUD_RESPONSE_TIME);
        CloudResponseTimeInstance cloudResponseTimeInstance = null;
        for (ServiceQualityInstance qosInstance : soaInstance
                .getServiceQualities()) {
            if (qosInstance instanceof CloudResponseTimeInstance) {
            	cloudResponseTimeInstance = ((CloudResponseTimeInstance) qosInstance);
            }
        }
        if (cloudResponseTimeInstance == null) {
        	cloudResponseTimeInstance = new CloudResponseTimeInstance();
        }
        cloudResponseTimeInstance.setMaxValueMs(cloudResponseTime);
        serviceQualityInstances.add(cloudResponseTimeInstance);
        
        //Container Response Time
        Float containerResponseTime = frontendModel.<Float> get(ApplicationModel.SLA_CONTAINER_RESPONSE_TIME);
        ContainerResponseTimeInstance containerResponseTimeInstance = null;
        for (ServiceQualityInstance qosInstance : soaInstance
                .getServiceQualities()) {
            if (qosInstance instanceof ContainerResponseTimeInstance) {
            	containerResponseTimeInstance = ((ContainerResponseTimeInstance) qosInstance);
            }
        }
        if (containerResponseTimeInstance == null) {
        	containerResponseTimeInstance = new ContainerResponseTimeInstance();
        }
        containerResponseTimeInstance.setMaxValueMs(containerResponseTime);
        serviceQualityInstances.add(containerResponseTimeInstance);
        
        //Memory Load
        Float memoryLoad = frontendModel.<Float> get(ApplicationModel.SLA_CONTAINER_RESPONSE_TIME);
        MemoryLoadInstance memoryLoadInstance = null;
        for (ServiceQualityInstance qosInstance : soaInstance
                .getServiceQualities()) {
            if (qosInstance instanceof MemoryLoadInstance) {
            	memoryLoadInstance = ((MemoryLoadInstance) qosInstance);
            }
        }
        if (memoryLoadInstance == null) {
        	memoryLoadInstance = new MemoryLoadInstance();
        }
        memoryLoadInstance.setMaxValueMB(memoryLoad);
        serviceQualityInstances.add(memoryLoadInstance);
