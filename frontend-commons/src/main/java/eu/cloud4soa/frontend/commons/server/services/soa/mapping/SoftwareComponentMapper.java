...
import static eu.cloud4soa.frontend.commons.client.datamodel.frontend.application.SoftwareComponentModel.DB_HOST;
...
import static eu.cloud4soa.frontend.commons.client.datamodel.frontend.application.SoftwareComponentModel.DB_PORT;
...
public class SoftwareComponentMapper extends
	...
	protected SoftwareComponentModel readFrom(SoftwareComponent soaInstance) {
		...
		if (soaInstance != null) {
			...
			if (soaInstance instanceof DBStorageComponent) {
				...
				//FIXME Obtaining DBInfo encoded in dbURL
				populateEncodedDBURLInformation(softwareComponentModel, dbStorageComponent);
	...
	private void populateEncodedDBURLInformation(SoftwareComponentModel softwareComponentModel,
			DBStorageComponent dbStorageComponent) {
		if (dbStorageComponent.getDBdeployment() == null) return;
		
		String dbUrlEncoded = dbStorageComponent.getDBdeployment().getUrl();
		int equalIndex = 0;
		int commaIndex = 0;
		
		equalIndex = dbUrlEncoded.indexOf('=', equalIndex);
		commaIndex = dbUrlEncoded.indexOf(',', commaIndex);
		String dbUrl = dbUrlEncoded.substring(equalIndex + 1, commaIndex);
		softwareComponentModel.set(DB_DEPLOYMENT_LOCATION_URL, dbUrl);
		
		equalIndex = dbUrlEncoded.indexOf('=', equalIndex + 1);
		commaIndex = dbUrlEncoded.indexOf(',', commaIndex + 1);
		String dbHost = dbUrlEncoded.substring(equalIndex + 1, commaIndex);
		softwareComponentModel.set(DB_HOST, dbHost);
		
		equalIndex = dbUrlEncoded.indexOf('=', equalIndex + 1);
		commaIndex = dbUrlEncoded.indexOf(',', commaIndex + 1);
		String dbPort = dbUrlEncoded.substring(equalIndex + 1, commaIndex);
		softwareComponentModel.set(DB_PORT, dbPort);
		
		equalIndex = dbUrlEncoded.indexOf('=', equalIndex + 1);
		commaIndex = dbUrlEncoded.indexOf(',', commaIndex + 1);
		String dbName = dbUrlEncoded.substring(equalIndex + 1, commaIndex);
		softwareComponentModel.set(DB_NAME, dbName);
		
		equalIndex = dbUrlEncoded.indexOf('=', equalIndex + 1);
		String dbUser = dbUrlEncoded.substring(equalIndex + 1);
		softwareComponentModel.set(DB_USER, dbUser);
	}
