...
public class DialogHelper {
	...
	public static Dialog showCommitRollbackMigrationDialog(String heading, String message) {
		final Dialog dialog = new Dialog();
		if (heading == null)
			heading = "Confirmation";
		dialog.setHeading(heading);
		dialog.setButtons(Dialog.YESNOCANCEL);
		dialog.getButtonById(Dialog.YES).setText("Commit");
		dialog.getButtonById(Dialog.NO).setText("Rollback");
		dialog.setBodyStyleName("pad-text");
		dialog.addText(message);
		dialog.getItem(0).getFocusSupport().setIgnore(true);
		dialog.setScrollMode(Scroll.AUTO);
