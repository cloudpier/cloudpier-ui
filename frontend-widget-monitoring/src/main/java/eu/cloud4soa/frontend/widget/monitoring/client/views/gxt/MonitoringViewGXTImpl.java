...
public class MonitoringViewGXTImpl extends LayoutContainer implements
	...
	private LayoutContainer monitoringPanel;
	private static String urlChart;
	...
	private Map<MonitoringMetricType, LayoutContainer> metricViews;
	...
	private void updateView() {
		...
		if (!areApplicationsRunning()) {
			for (LayoutContainer view: metricViews.values())
				view.removeAll();
		}
	...
	private void createView() {
		...
		monitoringPanel = new LayoutContainer();
		monitoringPanel.setLayout(new FlowLayout(5));
		...
		// Time frame section
		monitoringPanel.add(getTimeFrameSection());
		...
		//Available metrics
		Set<MonitoringMetricType> metricTypes = new HashSet<MonitoringMetricType>();
		for (ApplicationModel app: monitoredApplications){
			List<MonitoringMetricType> types = app.get(ApplicationModel.SUPPORTED_METRICS);
			metricTypes.addAll(types);
		}
		...
		metricViews = new HashMap <MonitoringMetricType, LayoutContainer> ();
		for (MonitoringMetricType type: metricTypes){			
			TabItem tabItem = new TabItem(type.getName());
			tabItem.setLayoutOnChange(true);
			tabItem.addListener(Events.Select,
					new Listener<ComponentEvent>() {
						@Override
						public void handleEvent(ComponentEvent be) {
							monitoringPanel.layout();
			...
			LayoutContainer metricView = getMetricView();
			metricViews.put(type, metricView);
			tabItem.add(metricView, new FitData(5));
			tabPanel.add(tabItem);
		}

		monitoringPanel.add(tabPanel);

		cp.add(monitoringPanel);
	}
	
	private LayoutContainer getMetricView() {
		LayoutContainer metricView = new LayoutContainer();
		metricView.setLayout(new FitLayout());
		metricView.setScrollMode(Scroll.AUTOY);
		metricView.setHeight(200);
		metricView.setBorders(true);
	...
	/**
	 * stats map application id to the statistics. Statistics map metric type to a list of metrics
	 */
	@Override
	public void renderMonitoringData(List<ApplicationModel> apps,
			Map<String, Map<MonitoringMetricType, List<IMonitoringMetric>>> stats) {

		// Guard Check monitoring stats
		boolean emptyStats = true;
		for (Map<MonitoringMetricType, List<IMonitoringMetric>> stat1: stats.values())
			for (List<IMonitoringMetric> stat : stat1.values()) {
				if (!stat.isEmpty()) {
					emptyStats = false;
					break;
				}
		...
		// Guard Remove empty statistics (if any occurs)
		Map<MonitoringMetricType, Map<String, List<IMonitoringMetric>>> newMetrics = new HashMap<MonitoringMetricType, Map<String, List<IMonitoringMetric>>>();
		
		List<ApplicationModel> applications = new ArrayList<ApplicationModel>();
		
		for (String appId : stats.keySet()) {
			boolean availableMetricsForApp = false;
			Map<MonitoringMetricType, List<IMonitoringMetric>> appMetrics = stats.get(appId);
			for (MonitoringMetricType type : appMetrics.keySet()) {
				List<IMonitoringMetric> metrics = appMetrics.get(type);
				if (!metrics.isEmpty()) {
					if (!newMetrics.containsKey(type))
							newMetrics.put (type, new HashMap<String, List<IMonitoringMetric>>());
					newMetrics.get(type).put(appId, metrics);
					availableMetricsForApp = true;
				} else {
					DialogHelper
							.showErrorDialog("There are not monitoring statistics for application "
									+ getApplicationById(apps, appId).getTitle() + " and metric type " + type.getName());
				}
			}
			if (availableMetricsForApp)
				applications.add(getApplicationById(apps, appId));
		}
		
		// Display new metrics
		for (MonitoringMetricType type : newMetrics.keySet()){
			if (type == MonitoringMetricType.UptimeInstance)
				renderPieChart(applications, type, newMetrics.get(type));
			else
				renderMetricsChart(applications, type, newMetrics.get(type));
		}

		layout();
	}
	
	private void renderMetricsChart(List<ApplicationModel> applications, MonitoringMetricType type,
			Map<String, List<IMonitoringMetric>> metrics) {
		
		Chart chart = new Chart(urlChart);
		chart.setHeight("200");
		...
		chartInstance = new ChartImpl(type.getName() + " for selected applications(" + type.getUnit() + ")",
			presenter.getMaxNumberMonitoringDisplayableValues());
		List<List<ChartPoint>> chartValues = chartInstance.setCharModelAxes(
				metrics, MAX_NUMBER_XAXIS_LABELS, MAX_NUMBER_YAXIS_LABELS);
		int i = 0;
		...
		chart.setChartModel(chartInstance.getChartModel());

		// ----------------------

		metricViews.get(type).removeAll();
		metricViews.get(type).add(chart);
	}

	private void renderPieChart(List<ApplicationModel> applications, MonitoringMetricType type, 
			Map<String, List<IMonitoringMetric>> metrics) {
		...
		for (ApplicationModel app : applications) {
			...
			ResponseCodePieChart pieChart = new ResponseCodePieChart(
					app.getTitle(), metrics.get(app.getKey()));
			chart.setChartModel(pieChart.getChartModel(app.getTitle()));

			metricViews.get(type).add(chart, new RowData(0.4, 1, new Margins(0)));
		}
